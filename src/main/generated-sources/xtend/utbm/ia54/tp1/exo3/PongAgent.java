...
public class PongAgent extends Agent {
  ...
  @Percept
  public void _handle_Ping_1(final Ping occurrence) {
    this.println(("Ping received #" + Integer.valueOf(occurrence.index)));
    Pong evt = new Pong(occurrence.index);
    UUID _iD = this.getID();
    Address _address = this.space.getAddress(_iD);
    evt.setSource(_address);
    this.println(("Pong emitted #" + Integer.valueOf(evt.index)));
    this.space.emit(evt);
  }
   ...
   */
  @Generated
  public PongAgent(final UUID parentID) {
    super(parentID, null);
  }
